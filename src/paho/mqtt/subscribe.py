...
def _on_connect(client, userdata, flags, reason_code, properties):
    ...
    if isinstance(userdata['topics'], list):
        for topic in userdata['topics']:
            client.subscribe(topic, userdata['qos'], user_properties=userdata['user_properties'])
    else:
        client.subscribe(userdata['topics'], userdata['qos'], user_properties=userdata['user_properties'])
...
def callback(callback, topics, qos=0, userdata=None, hostname="localhost",
    ...
    :param dict user_properties: (MQTT v5.0 only) user properties to merge into
           the subscribe properties automatically.
    """
